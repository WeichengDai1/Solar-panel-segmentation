...
#         xup = self.dropout(self.bn6(self.upconv6(x4)))# x7in
...
def main():
    ...
    num_epochs = 500  # The number of times entire dataset is trained
    ...
    optimizer = optim.Adam(unet.parameters(), lr=1e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[50, 100, 150, 200, 250, 300, 350, 400, 450])
    # unet.to(device)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for step, (batch_x, batch_y) in enumerate(tqdm(loader)):
            ...
            scheduler.step()
        print(sum(list_loss) / len(list_loss))
        print(sum(iou_list) / len(iou_list))
        # break
        loss_dict[epoch]=(sum(list_loss) / len(list_loss))
        correct_dict[epoch] = (sum(iou_list) / len(iou_list))
        torch.save(unet.state_dict(), 'unetModel/new_%s.pt' % (epoch))

    plt.title('IOU train data')
    plt.xlabel('epoch')
    plt.ylabel('IOU')
    plt.plot(range(num_epochs), correct_dict.values(), label='IOU')
    plt.legend()
    plt.savefig('IOUtraining_UNet.jpg')
    plt.clf()
    ...
    plt.plot(range(num_epochs), loss_dict.values(), label='loss')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.legend()
    plt.savefig('Losstrain_UNet.jpg')
